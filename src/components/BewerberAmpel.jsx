...
// Gleiche Farbe nochmal klicken = zurücksetzen
if (ampelWert === farbe) {
onAmpelChange(null);
return;
}
if (farbe === ‘gruen’) {
// Grün braucht keine Notiz – direkt speichern
onAmpelChange(‘gruen’);
} else {
// Gelb/Rot: erst Overlay zeigen, Ampel erst nach Notiz setzen
setNotizOverlay(farbe);
}
};

const handleNotizGespeichert = (notizDaten) => {
// Zuerst Notiz speichern, dann Ampelfarbe setzen
...
// Overlay schließen ohne Ampel zu setzen
setNotizOverlay(null);
};
...
className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all text-white ${ ampelWert === farbe ? `${bg} ring-2 ${ring} scale-105 shadow-sm`:`${bg} opacity-30 ${hover} hover:opacity-60` }`}>
{label}
</button>
))}
</div>

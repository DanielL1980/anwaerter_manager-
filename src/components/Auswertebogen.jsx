...
function KategorieBlock({ kategorie, auswertung, alleOffen, onPunkteChange, onNotizChange }) {
  ...
  // Sync mit "Alle öffnen/schließen"
  useEffect(() => {
    if (alleOffen === true) setOffen(true);
  ...
  const total = kategorie.punkte.length;
  const vollstaendig = bewerteteAnzahl === total;

  return (
    <div className="card overflow-hidden print-container auswertebogen-kategorie">
      {/* Klappbarer Header */}
      <button
        onClick={() => setOffen(!offen)}
        className="w-full bg-gradient-to-r from-slate-700 to-slate-600 px-5 py-4 flex items-center justify-between text-left hover:from-slate-600 hover:to-slate-500 transition-all">
        <div className="flex items-center gap-3">
          <ChevronDown size={18} className={`text-white/70 transition-transform flex-shrink-0 ${offen ? 'rotate-180' : ''}`} />
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {kategorie.gewichtung > 1 && (
            <span className="bg-white/20 text-white text-xs font-bold px-2 py-1 rounded-lg">
              {kategorie.gewichtung}-fach
            </span>
          )}
          <span className={`text-xs font-bold px-2 py-1 rounded-lg ${vollstaendig ? 'bg-emerald-500 text-white' : 'bg-white/20 text-white/80'}`}>
            {bewerteteAnzahl}/{total}
          </span>
        </div>
      </button>
      ...
      {offen && (
        <div className="divide-y divide-slate-100">
          {kategorie.punkte.map(punkt => {
            ...
            return (
              <div key={kriteriumId} className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3 items-start">
                <p className="font-medium text-slate-700 self-center text-sm">{punkt.text}</p>
                <div>
                    ...
                    {PUNKTE_SKALA.map(skala => (
                      <label key={skala.value} className="cursor-pointer">
                        <input type="radio" name={kriteriumId} value={skala.value}
                          ...
                          onChange={() => onPunkteChange(kriteriumId, skala.value)}
                  ...
                  <textarea
                    ...
                    onChange={(e) => onNotizChange(kriteriumId, e.target.value)}
                    placeholder="Notizen..."
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end gap-2">
        <button onClick={() => setAlleOffen(true)}
        ...
        <button onClick={() => setAlleOffen(false)}
          className="text-xs text-slate-500 hover:text-slate-700 font-medium px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 transition">
          Alle schließen
        </button>
      </div>
      ...
      {kriterien.map(kategorie => (
        <KategorieBlock
          ...
          alleOffen={alleOffen}
          onPunkteChange={handlePunkteChange}
          onNotizChange={handleNotizChange}
        />
      ))}
